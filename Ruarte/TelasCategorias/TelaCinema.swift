...
struct TelaCinema: View {
    ...
    var body: some View {
        NavigationStack {
            ScrollView{
                NavigationLink(destination: {TelaLugares(place: allPlaces[12], viewModel: PlaceViewModel(places: allPlaces))}, label: {
                    
                    AsyncImage(url: URL(string: allPlaces[12].images[0])) { image in
                        image
                            .resizable()
                            .scaledToFill()
